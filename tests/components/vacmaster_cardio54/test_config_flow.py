...
async def test_user_flow_pair_send_failure(
    ...
    """If the pair burst fails the flow shows the pair_failed menu."""
    entity_id = _transmitter_entity_id(hass, mock_rf_entity)
    ...
    with patch(
        ...
        with patch(
            "custom_components.vacmaster_cardio54.config_flow.async_send_command",
...
async def test_user_flow_recover_after_pair_failure(
    ...
    """Retry from pair_failed runs the pair step again and can succeed."""
    entity_id = _transmitter_entity_id(hass, mock_rf_entity)
    ...
    with patch(
        ...
        # async_step_retry routes us back into pair. We need to submit it.
        if result["type"] is FlowResultType.FORM and result["step_id"] == "pair":
            result = await hass.config_entries.flow.async_configure(
                result["flow_id"], {}
            )
        assert result["type"] is FlowResultType.MENU
...
async def test_user_flow_test_send_failure(
    ...
    """If the post-pair test send fails we land on pair_failed too."""
    entity_id = _transmitter_entity_id(hass, mock_rf_entity)
